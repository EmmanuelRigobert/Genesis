...
public class Aufgabe2 {
	...
	/**
	 * Converts an RGB color to a gray scale.
	 * 
	...
	public static int[][] convertPictureToGrayscale(int[][] pixels) {
		...
		for(int x=0; x<pixels.length;x++){
			for(int y=0; y <pixels[x].length;y++){
				...
				gray = convertColorToGrayscale(red+green+blue);
				pixels[x][y] = Colors.toRGB(gray, gray, gray); // setzt den Farbwert des Pixels an der Stelle x, y auf
			}
		}
		return pixels;
	}
		...
		for(int x=0; x<pixels.length;x++){
			for(int y=0; y <pixels[x].length;y++){
				...
				if(x<pixels.length/3){
					pixels[x][y] = Colors.toRGB(red, 0, 0); // setzt den Farbwert des Pixels an der Stelle x, y auf
				}else if( pixels.length / 3 <=x && x < pixels.length*2/3){
					pixels[x][y] = Colors.toRGB(0, green, 0); // setzt den Farbwert des Pixels an der Stelle x, y auf
				}else if (pixels.length * 2 / 3 <= x){
					pixels[x][y] = Colors.toRGB(0, 0, blue); // setzt den Farbwert des Pixels an der Stelle x, y auf
				}
			}
		}
		return pixels;
	...
	public static  int[][] drawBorderOnPicture(int[][] pixels, int borderWidth, int borderColor) {
		...
		for(int j = 0; j<borderWidth;j++) {
			for (int x = 0; x < pixels.length; x++) {
				for (int y = 0; y < pixels[x].length; y++) {
					if(x==j || y==j || x== pixels.length-1-j || y== pixels[x].length-1-j ){
						pixels[x][y] = borderColor; // setzt den Farbwert des Pixels an der Stelle x, y auf
					}
				}
			}
		}
	...
	public static  int[][] extendPictureWithBorder(int[][] pixels, int borderWidth, int borderColor) {
		// TODO: implement me!
		int[][] newArray = new int[pixels.length + (borderWidth*2)][pixels[0].length+(borderWidth*2)];
		for (int x = 0; x < newArray.length; x++) {
			for (int y = 0; y < newArray[0].length; y++) {
					newArray[x][y]=borderColor;

			}
		}
		for (int x = 0; x < pixels.length; x++) {
			for (int y = 0; y < pixels[x].length; y++) {
				newArray[x+borderWidth][y+borderWidth]=pixels[x][y];
			}
		}
		return newArray;
	...
	public static int[][] rotatePicture(int[][] pixels) {
		...
		int j=pixels[0].length-1;
		int k=0;
		for (int x = 0; x < pixels.length; x++) {
			for (int y = 0; y < pixels[x].length; y++) {
				newArray[y][pixels.length - 1 - x] = pixels[x][y];
			}
		}
		return newArray;
	}
	...
	public static int[][] shrinkPicture(int[][] pixels, int n) {
		// TODO: implement me!
		int[][] newArray = new int[computeShrinkedSize(pixels.length, n)][computeShrinkedSize(pixels[0].length, n)];
		// Map pixels from the original image to the new image
		for (int x = 0; x < newArray.length; x++) {
			for (int y = 0; y < newArray[x].length; y++) {
				newArray[x][y] = pixels[x * n][y * n];
			}
		}
		return newArray;
	}
...
//		miniproject1.Images.writeImage(name + "-gray-" + timestamp + extension, convertPictureToGrayscale(miniproject1.Images.readImage(input)));
//		miniproject1.Images.writeImage(name + "-warhol-" + timestamp + extension, andyWarhol(miniproject1.Images.readImage(input)));
//		miniproject1.Images.writeImage(name + "-draw-" + timestamp + extension, drawBorderOnPicture(miniproject1.Images.readImage(input), 100, miniproject1.Colors.toRGB(255, 0, 0)));
...
//		miniproject1.Images.writeImage(name + "-rotate-" + timestamp + extension, rotatePicture(miniproject1.Images.readImage(input)));
//		miniproject1.Images.writeImage(name + "-shrink-" + timestamp + extension, shrinkPicture(miniproject1.Images.readImage(input), 3));
//		miniproject1.Images.writeImage(name + "-smooth-" + timestamp + extension, shrinkPictureSmooth(miniproject1.Images.readImage(input), 3));
	}
}
